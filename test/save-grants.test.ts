...
function execute(grants: HostSaveGrants, runtime: BackendRuntime, operation: Parameters<HostSaveGrants["authorize"]>[0], input: unknown) {
  return grants.authorize(operation, input) ?? runtime[operation](input as never)
}
...
test("save grants bind exact operations, expire, revoke, invalidate and stay bounded", async (t) => {
  ...
  assert.equal(errorCode(grants.authorize("saveSession", { sessionId: "missing", expectedRevision: 0 })), "save_session_unavailable")
  ...
  const first = approval(execute(grants, runtime, "saveSession", input))
  assert.equal(approval(execute(grants, runtime, "saveSession", input)).approvalRequestId, first.approvalRequestId)
  assert.equal(approval(execute(grants, runtime, "saveSession", { ...input, force: false })).approvalRequestId, first.approvalRequestId)
  ...
  for (const changed of [{ ...input, sessionId: "two" }, { ...input, force: true }, { ...input, mode: "materializeCleanSession" }, { ...input, targetPath: "another.fairy" }]) {
    assert.notEqual(approval(execute(grants, runtime, "saveSession", changed)).approvalRequestId, first.approvalRequestId)
  }
  const materialize = approval(execute(grants, runtime, "materializeSession", input))
  assert.notEqual(materialize.operationDigest, first.operationDigest)
  assert.notEqual(approval(execute(grants, runtime, "materializeSession", { ...input, reason: "different operation" })).operationDigest, materialize.operationDigest)
  assert.equal(errorCode(execute(grants, runtime, "saveSession", { sessionId: "one" })), "save_input_invalid")
  assert.equal(errorCode(execute(grants, runtime, "saveSession", { ...input, expectedRevision: 1 })), "save_revision_stale")
  assert.equal(errorCode(execute(grants, runtime, "saveSession", { ...input, fileSystem: {} })), "save_input_invalid")
  assert.equal(errorCode(execute(grants, runtime, "materializeSession", { ...input, storage: {} })), "save_input_invalid")
  assert.equal(errorCode(execute(grants, runtime, "materializeSession", { ...input, reason: "x".repeat(1_001) })), "save_input_invalid")
  ...
  const attempts = await Promise.all([execute(grants, runtime, "saveSession", input), execute(grants, runtime, "saveSession", input)])
  assert.equal(attempts.filter((result) => result.ok).length, 1)
  assert.equal(save.mock.callCount(), 1)

  const closingInput = { sessionId: "two", expectedRevision: 0 }
  const closingRequest = approval(execute(grants, runtime, "saveSession", closingInput))
  grants.decide(closingRequest.approvalRequestId, "approve")
  grants.beginClose("two")
  grants.beginClose("two")
  assert.equal(errorCode(execute(grants, runtime, "saveSession", closingInput)), "save_session_closing")
  grants.endClose("two")
  assert.equal(errorCode(execute(grants, runtime, "saveSession", closingInput)), "save_session_closing")
  grants.endClose("two")
  assert.notEqual(approval(execute(grants, runtime, "saveSession", closingInput)).approvalRequestId, closingRequest.approvalRequestId)
  runtime.openProjectSession({ project, sessionId: "long-target", canonicalProjectPath: "x".repeat(4_097) })
  assert.equal(errorCode(execute(grants, runtime, "saveSession", { sessionId: "long-target", expectedRevision: 0 })), "save_target_invalid")
  ...
  assert.notEqual(approval(execute(grants, runtime, "saveSession", input)).approvalRequestId, retry.approvalRequestId)
  ...
  assert.notEqual(approval(execute(grants, runtime, "saveSession", input)).approvalRequestId, pending.approvalRequestId)
  clock.mock.restore()
  const stale = approval(execute(grants, runtime, "saveSession", { ...input, force: true }))
  ...
  assert.equal(errorCode(execute(grants, runtime, "saveSession", { ...input, force: true })), "save_revision_stale")
  assert.equal(save.mock.callCount(), 1)

  const bounded = new HostSaveGrants(runtime)
  for (let i = 0; i < MAX_SAVE_APPROVALS; i++) approval(execute(bounded, runtime, "materializeSession", { ...input, expectedRevision: 1, reason: String(i) }))
  assert.equal(errorCode(execute(bounded, runtime, "materializeSession", { ...input, expectedRevision: 1, reason: "overflow" })), "save_approval_limit")
  const expired = bounded.list()[0]
  bounded.decide(expired.approvalRequestId, "reject")
  approval(execute(bounded, runtime, "materializeSession", { ...input, expectedRevision: 1, reason: "room after rejection" }))
...
test("a failed or throwing backend attempt consumes its grant and never auto-retries", async (t) => {
  ...
  const first = approval(execute(grants, runtime, "materializeSession", input))
  grants.decide(first.approvalRequestId, "approve")
  assert.equal(errorCode(await execute(grants, runtime, "materializeSession", input)), "capability_unavailable")
  assert.equal(grants.list()[0].status, "consumed")
  assert.notEqual(approval(execute(grants, runtime, "materializeSession", input)).approvalRequestId, first.approvalRequestId)
  const throwing = approval(execute(grants, runtime, "saveSession", input))
  ...
  assert.throws(() => execute(grants, runtime, "saveSession", input), /injected write failure/)
  approval(execute(grants, runtime, "saveSession", input))
...
test("an edit already queued in the backend cannot be saved using an older approved revision", async () => {
  ...
  const request = approval(execute(grants, runtime, "materializeSession", input))
  ...
  const save = execute(grants, runtime, "materializeSession", input)
  assert.equal(grants.list()[0].status, "consumed")
